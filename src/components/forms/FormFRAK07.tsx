...
export const DEFAULT_ADJUSTMENT_OPTIONS = [
  ...
  { 
    id: 'adj4', 
    label: 'Pelaksanaan asesmen secara fleksibel karena alasan keletihan atau keperluan pengobatan',
    options: [
...
export interface FormFRAK07Props {
  ...
  onAdjustmentChange?: (id: string, field: 'required' | 'note' | 'selectedOptions', value: any) => void;
...
export function FormFRAK07(props: FormFRAK07Props) {
  ...
  const noAdjustment = props.noAdjustment !== undefined ? props.noAdjustment : localNoAdjustment;
  const adjustments = props.adjustments !== undefined ? props.adjustments : localAdjustments;
  const acuanPembanding = props.acuanPembanding !== undefined ? props.acuanPembanding : localAcuan;
  const metodeAsesmen = props.metodeAsesmen !== undefined ? props.metodeAsesmen : localMetode;
  const instrumenAsesmen = props.instrumenAsesmen !== undefined ? props.instrumenAsesmen : localInstrumen;
  ...
  const handleNoAdjToggle = (checked: boolean) => {
    ...
    DEFAULT_ADJUSTMENT_OPTIONS.forEach(opt => {
      newAdj[opt.id] = { required: checked ? false : null, note: '', selectedOptions: [] };
    });
  ...
  const handleAdjChangeInternal = (id: string, field: 'required' | 'note' | 'selectedOptions', value: any) => {
    if (props.onAdjustmentChange) {
    ...
    <div className="animate-in fade-in duration-300">
      ...
      <div className="border border-slate-300 mb-6 bg-white">
        ...
        <div className="p-4 text-xs md:text-sm space-y-2">
          <ul className="list-disc pl-5 space-y-1 text-slate-800 font-medium">
            ...
            <li>Berilah tanda <span className="inline-block border border-slate-600 px-1 text-[10px] mx-1">v</span> pada kotak <span className="inline-block border border-slate-600 px-1 text-[10px] mx-1 font-sans">☐</span> pada kolom potensi asesi.</li>
            <li>Berilah tanda v Ya atau Tidak pada tanda ** sesuai pilihan, jika jawaban Ya selanjutnya pada kolom keterangan berilah tanda v di kotak <span className="inline-block border border-slate-600 px-1 text-[10px] mx-1 font-sans">☐</span> yang tersedia, pilihan boleh lebih dari satu.</li>
          </ul>
        </div>
      </div>
      ...
      <div className="mb-6">
        <label className="flex items-center gap-3 p-4 border border-slate-300 bg-white cursor-pointer">
          ...
          />
          <span className="font-bold text-slate-900">Tidak ada penyesuaian yang diperlukan</span>
        </label>
      </div>
        ...
        <table className="w-full border-collapse text-sm min-w-[600px]">
          <thead>
            <tr className="bg-white border-b border-slate-300 text-center">
              ...
              <th className="border border-slate-300 p-3 text-left">Mengidentifikasi Persyaratan (Karakteristik Asesi)</th>
              <th className="border border-slate-300 p-3 w-32">Diperlukan Penyesuaian?</th>
              <th className="border border-slate-300 p-3 w-1/3 text-left">Keterangan / Catatan</th>
            </tr>
          </thead>
          <tbody>
            {DEFAULT_ADJUSTMENT_OPTIONS.map((opt, idx) => (
              <tr key={opt.id}>
                <td className="border border-slate-300 p-3 text-center align-top">{idx + 1}</td>
                <td className="border border-slate-300 p-3 align-top">{opt.label}</td>
                  ...
                  <div className="flex flex-col gap-2 items-center">
                    <label className="flex items-center gap-1 cursor-pointer">
                      <input 
                        ...
                        onChange={() => handleAdjChangeInternal(opt.id, 'required', true)}
                    ...
                    <label className="flex items-center gap-1 cursor-pointer">
                      <input 
                        ...
                        onChange={() => handleAdjChangeInternal(opt.id, 'required', false)}
                        disabled={noAdjustment || props.readOnly}
                      /> Tidak
                    </label>
                  </div>
                ...
                <td className="border border-slate-300 p-3 align-top">
                  <div className="space-y-2 mb-3">
                    {opt.options.map((o, oidx) => (
                      <label key={oidx} className="flex items-start gap-2 text-xs cursor-pointer">
                        <input 
                          type="checkbox" 
                          className="mt-0.5 shrink-0"
                          checked={adjustments[opt.id]?.selectedOptions?.includes(o) || false}
                  ...
                  <input 
                    ...
                    onChange={(e) => handleAdjChangeInternal(opt.id, 'note', e.target.value)}
                    disabled={noAdjustment || props.readOnly}
                  />
                </td>
              </tr>
      ...
      <div className="border border-slate-300 mt-8 mb-6 p-4">
        <h4 className="font-bold text-sm mb-4">Hasil Penyesuaian yang wajar dan beralasan disepakati menggunakan :</h4>
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            <span className="w-6 font-bold text-sm">1)</span>
            <span className="w-56 font-bold text-sm">Acuan Pembanding Asesmen</span>
            <span className="font-bold">:</span>
            <input 
              type="text" 
              value={acuanPembanding} 
              disabled={props.readOnly || props.isAsesi}
              onChange={(e) => props.onAcuanPembandingChange ? props.onAcuanPembandingChange(e.target.value) : setLocalAcuan(e.target.value)} 
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            ...
            <input 
              ...
              value={metodeAsesmen} 
              disabled={props.readOnly || props.isAsesi}
              onChange={(e) => props.onMetodeAsesmenChange ? props.onMetodeAsesmenChange(e.target.value) : setLocalMetode(e.target.value)} 
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            ...
            <input 
              ...
              disabled={props.readOnly || props.isAsesi}
              onChange={(e) => props.onInstrumenAsesmenChange ? props.onInstrumenAsesmenChange(e.target.value) : setLocalInstrumen(e.target.value)} 
      ...
      <div className="border border-slate-300 mb-8 grid grid-cols-1 md:grid-cols-2">
          ...
          <div>
            <div className="flex gap-2">
              ...
              <input 
                ...
                value={asesorName} 
                disabled={props.readOnly || props.isAsesi}
                onChange={(e) => props.onAsesorNameChange ? props.onAsesorNameChange(e.target.value) : setLocalAsesorName(e.target.value)} 
        ...
        <div className="p-4 flex flex-col justify-between items-start w-full">
          <span className="font-bold text-sm mb-4">Tanggal dan Tanda Tangan Asesor:</span>
          <div className="mb-4">
            {asesorSignature ? (
              <img 
                src={asesorSignature} 
          ...
          <div className="w-full">
            <input 
              ...
              value={asesorDate} 
              disabled={props.readOnly || props.isAsesi}
              onChange={(e) => props.onAsesorDateChange ? props.onAsesorDateChange(e.target.value) : setLocalAsesorDate(e.target.value)} 
          ...
          <div>
            <div className="flex gap-2">
              ...
              <input 
                ...
                value={asesiName} 
                disabled={props.readOnly}
                onChange={(e) => props.onAsesiNameChange ? props.onAsesiNameChange(e.target.value) : setLocalAsesiName(e.target.value)} 
                className="flex-1 border-b border-slate-300 outline-none focus:border-slate-800 bg-transparent text-sm font-medium"
              />
            </div>
          </div>
        ...
        <div className="p-4 border-t border-slate-300 flex flex-col justify-between items-start w-full">
          <span className="font-bold text-sm mb-4">Tanggal dan Tanda Tangan Asesi:</span>
          <div className="mb-4">
            {asesiSignature ? (
              <img 
          ...
          <div className="w-full">
            <input 
              ...
              value={asesiDate} 
              disabled={props.readOnly}
              onChange={(e) => props.onAsesiDateChange ? props.onAsesiDateChange(e.target.value) : setLocalAsesiDate(e.target.value)} 
              className="w-full border-b border-slate-300 outline-none focus:border-slate-800 bg-transparent text-sm font-medium py-1" 
            />
          </div>
        </div>
