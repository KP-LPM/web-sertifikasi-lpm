...
export const DEFAULT_STEP3_QUESTIONS = [
  { id: 's3_q1', skenario: 'Selama menerjemahkan teks panduan teknis, Anda menemui istilah khusus industri yang tidak ada di kamus umum dan alat bantu konvensional.', pertanyaan: 'Bagaimana Anda akan mencari padanan istilah tersebut dan mendokumentasikan prosesnya?', elemen: 'E1/KUK 1.1; E2/KUK 2.1' },
  { id: 's3_q2', skenario: 'Anda telah menemukan beberapa referensi dari alat bantu nonkonvensional (misalnya forum profesi atau korpus).', pertanyaan: 'Bagaimana Anda merangkum dan memverifikasi ketepatan makna ungkapan tersebut sebelum menggunakannya dalam terjemahan?', elemen: 'E1/KUK 1.2; E2/KUK 2.2' },
  { id: 's3_q3', skenario: 'Klien meminta agar teks diterjemahkan dengan gaya bahasa santai untuk pembaca remaja, meskipun teks aslinya agak formal.', pertanyaan: 'Metode penerjemahan apa yang akan Anda pilih dan bagaimana Anda menerapkannya?', elemen: 'E1/KUK 1.1; E2/KUK 2.2' },
  { id: 's3_q4', skenario: 'Anda menemui kalimat idiomatik yang jika diterjemahkan harfiah akan terdengar kaku di bahasa sasaran.', pertanyaan: 'Teknik penerjemahan apa yang akan Anda gunakan pada frasa atau klausa tersebut agar alami namun maknanya tersampaikan?', elemen: 'E1/KUK 1.2; E2/KUK 2.1' }
];
...
export interface FormFRIA04BProps {
  ...
  step3Answers?: Record<string, { answer: string; achievement: boolean | null }>;
  onAnswerChange?: (id: string, field: 'answer' | 'achievement', value: any) => void;
  onStep3Change?: (id: string, field: 'answer' | 'achievement', value: any) => void;
  rekomendasi?: string;
  rekomendasiStep3?: string;
  onRekomendasiChange?: (val: any) => void;
  onRekomendasiStep3Change?: (val: any) => void;
  asesiName?: string;
  ...
  penyusunStep3?: Array<{ nama: string; noMet: string; ttdTanggal: string }>;
  onPenyusunChange?: (penyusun: Array<{ nama: string; noMet: string; ttdTanggal: string }>) => void;
  onPenyusunStep3Change?: (penyusun: Array<{ nama: string; noMet: string; ttdTanggal: string }>) => void;
  validator?: Array<{ nama: string; noMet: string; ttdTanggal: string }>;
  validatorStep3?: Array<{ nama: string; noMet: string; ttdTanggal: string }>;
  onValidatorChange?: (validator: Array<{ nama: string; noMet: string; ttdTanggal: string }>) => void;
  onValidatorStep3Change?: (validator: Array<{ nama: string; noMet: string; ttdTanggal: string }>) => void;
  readOnly?: boolean;
  isAsesi?: boolean;
  showHeader?: boolean;
  onNext?: () => void;
...
export function FormFRIA04B(props: FormFRIA04BProps) {
  const questions = props.step3Questions || props.questions || DEFAULT_STEP3_QUESTIONS;

  const [localAnswers, setLocalAnswers] = useState<Record<string, { answer: string; achievement: boolean | null }>>({});
  const [localRekomendasi, setLocalRekomendasi] = useState<'Kompeten' | 'Belum Kompeten' | ''>('');
  ...
  const rekomendasi = props.rekomendasiStep3 !== undefined ? props.rekomendasiStep3 : (props.rekomendasi !== undefined ? props.rekomendasi : localRekomendasi);
  const asesiName = props.asesiNameStep3 !== undefined ? props.asesiNameStep3 : (props.asesiName !== undefined ? props.asesiName : localAsesiName);
  const asesiSignature = props.asesiSignatureStep3 !== undefined ? props.asesiSignatureStep3 : (props.asesiSignature !== undefined ? props.asesiSignature : localAsesiSig);
  const asesiDate = props.asesiDateStep3 !== undefined ? props.asesiDateStep3 : (props.asesiDate !== undefined ? props.asesiDate : localAsesiDate);
  const asesorName = props.asesorNameStep3 !== undefined ? props.asesorNameStep3 : (props.asesorName !== undefined ? props.asesorName : localAsesorName);
  const asesorReg = props.asesorRegStep3 !== undefined ? props.asesorRegStep3 : (props.asesorReg !== undefined ? props.asesorReg : localAsesorReg);
  const asesorSignature = props.asesorSignatureStep3 !== undefined ? props.asesorSignatureStep3 : (props.asesorSignature !== undefined ? props.asesorSignature : localAsesorSig);
  const asesorDate = props.asesorDateStep3 !== undefined ? props.asesorDateStep3 : (props.asesorDate !== undefined ? props.asesorDate : localAsesorDate);
  ...
  const handleAsesiSignatureChange = (val: string) => {
    if (props.onAsesiSignatureStep3Change) props.onAsesiSignatureStep3Change(val);
  ...
  const handleAsesorSignatureChange = (val: string) => {
    if (props.onAsesorSignatureStep3Change) props.onAsesorSignatureStep3Change(val);
  ...
  const lingkupList = [
    { title: "Mencari Makna Kata dan Ungkapan Menggunakan Alat Bantu", qs: [questions[0], questions[1]].filter(Boolean) },
    { title: "Memilih Metode Penerjemahan yang Tepat", qs: [questions[2], questions[3]].filter(Boolean) },
    { title: "Memilih Teknik Penerjemahan", qs: [questions[4], questions[5]].filter(Boolean) },
    { title: "Penilaian Hasil Terjemahan", qs: [questions[6], questions[7]].filter(Boolean) },
    { title: "Penyusunan Glosarium", qs: [questions[8], questions[9]].filter(Boolean) }
    ...
    <div className="animate-in fade-in duration-300">
      ...
      <div className="border border-slate-300 mb-6 bg-white">
        ...
        <div className="p-4 text-xs md:text-sm space-y-2">
          <ul className="list-disc pl-5 space-y-1 text-slate-800 font-medium">
            ...
            <li>Penilaian dilakukan sesuai dengan <strong>FR IA 04A. DIT. Daftar Instruksi Terstruktur (Penjelasan Proyek Singkat/ Kegiatan Terstruktur Lainnya)</strong></li>
        ...
        <table className="w-full border-collapse min-w-[650px]">
          <thead>
            <tr className="bg-white border-b border-slate-300 text-center">
              <th className="border border-slate-300 p-2" colSpan={3}>Aspek Penilaian</th>
              <th className="border border-slate-300 p-2" colSpan={2}>Pencapaian</th>
            </tr>
            <tr className="bg-white text-center">
              <th className="border border-slate-300 p-2 font-bold w-1/4">Lingkup Penyajian proyek atau kegiatan terstruktur lainnya</th>
              <th className="border border-slate-300 p-2 font-bold w-1/2">Daftar Pertanyaan</th>
              <th className="border border-slate-300 p-2 font-bold w-32">Kesesuaian dengan standar kompetensi kerja (unit/elemen/KUK)</th>
              <th className="border border-slate-300 p-2 font-bold w-12">Ya</th>
              <th className="border border-slate-300 p-2 font-bold w-12">Tdk</th>
            </tr>
          </thead>
          <tbody>
            {lingkupList.map((lingkup, lIdx) => (
              <React.Fragment key={lIdx}>
                {lingkup.qs.map((q, qIdx) => (
                  <tr key={q.id}>
                    {qIdx === 0 && (
                      <td className="border border-slate-300 p-2 align-top" rowSpan={lingkup.qs.length}>
                    ...
                    <td className="border border-slate-300 p-3 align-top space-y-2">
                      ...
                      <div>{lIdx * 2 + qIdx + 1}. Skenario: {q.skenario}</div>
                      <div>Pertanyaan: {q.pertanyaan}</div>
                      <div className="mt-4">
                        <div className="font-bold mb-1">Tanggapan:</div>
                        <textarea 
                          disabled={props.readOnly || props.isAsesi}
                          className="w-full border border-slate-300 outline-none focus:border-slate-800 p-2 bg-transparent min-h-[100px]"
      ...
      <div className="mt-8 border border-slate-300 bg-white text-sm">
        <table className="w-full border-collapse">
          <tbody>
            <tr>
              <td className="border border-slate-300 p-3 font-bold w-1/3 align-top">Rekomendasi Asesor:</td>
              <td className="border border-slate-300 p-3 align-top space-y-2">
                <div>Asesi telah memenuhi/belum memenuhi pencapaian seluruh kriteria unjuk kerja, direkomendasikan:</div>
