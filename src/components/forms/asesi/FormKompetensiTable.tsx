...
import { Eye, Upload, Info } from "lucide-react";
...
export function FormKompetensiTable({
  ...
  const groupedList = kompetensiList.reduce(
    (acc, comp) => {
  ...
  const currentRecords = unitEntries.slice(
      ...
      {infoText && (
        <div className="bg-sky-50/50 border-b border-sky-100 p-4">
          <div className="flex gap-3 max-w-4xl">
            ...
            <p className="text-sm text-slate-600 font-medium">{infoText}</p>
          </div>
        </div>
      )}
        ...
        <table className="w-full text-left border-collapse min-w-[1000px]">
          <thead>
            <tr className="bg-[#0F172A] border-b border-[#0F172A]">
              <th className="px-4 py-3 md:px-6 md:py-4 align-top w-1/4 sticky top-0 z-20 bg-[#0F172A] text-xs font-bold text-white/90 uppercase tracking-wider">
                Unit Kompetensi
              </th>
              <th className="px-4 py-3 md:px-6 md:py-4 align-top w-1/4 sticky top-0 z-20 bg-[#0F172A] text-xs font-bold text-white/90 uppercase tracking-wider">
                Elemen
              </th>
              <th className="px-4 py-3 md:px-6 md:py-4 align-top w-1/3 sticky top-0 z-20 bg-[#0F172A] text-xs font-bold text-white/90 uppercase tracking-wider">
                Kriteria Unjuk Kerja
              </th>
              <th className="px-4 py-3 md:px-6 md:py-4 align-top w-48 sticky right-0 bg-[#0F172A] z-30 shadow-[-6px_0_15px_-4px_rgba(0,0,0,0.06)] sticky top-0 text-xs font-bold text-white/90 uppercase tracking-wider border-l border-white/10">
                Bukti Portofolio
              </th>
            </tr>
          </thead>
                  ...
                  {unit.elements.map((comp, eIdx) => {
                    ...
                    return (
                      ...
                      >
                        {eIdx === 0 && (
                          <td
                            className="px-4 py-4 md:px-6 md:py-5 align-top border-r border-gray-200"
                            rowSpan={unit.elements.length}
                          >
      ...
      {totalPages >= 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 py-3 md:px-6 md:py-4 border-t border-slate-200 bg-white">
          <span className="text-xs md:text-sm text-slate-500">
            ...
            hingga{" "}
            <span className="font-semibold text-slate-700">
              {Math.min(currentPage * itemsPerPage, unitEntries.length)}
            </span>{" "}
            dari{" "}
            <span className="font-semibold text-slate-700">
              {unitEntries.length}
