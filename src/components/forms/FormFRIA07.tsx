...
export const DEFAULT_STEP4_QUESTIONS = [
  { id: 's4_q1', pertanyaan: 'Mengapa penting untuk merancang profil sasaran pembaca mencakup semua informasi yang diperlukan?', elemen: 'M.74PEN01.008.1 E1/KUK 1.1; E1/KUK 1.2', kunci: 'Merancang profil sasaran dengan detail penting untuk mendukung pemilihan metode dan gaya bahasa yang tepat.' },
  { id: 's4_q2', pertanyaan: 'Bagaimana cara memastikan alat bantu penerjemahan nonkonvensional yang digunakan sudah tepat?', elemen: '-', kunci: 'Memastikan alat bantu tepat dengan cara menguji akurasi terjemahan pada sebagian teks dan membandingkannya dengan referensi paralel.' },
  { id: 's4_q3', pertanyaan: 'Mengapa penting untuk menyusun daftar glosarium sebelum menerjemahkan teks panjang?', elemen: 'M.74PEN01.002.1 E1/KUK 1.1; E2/KUK 2.2', kunci: 'Menyusun glosarium penting untuk menjaga konsistensi istilah teknis di seluruh dokumen terjemahan.' },
  { id: 's4_q4', pertanyaan: 'Bagaimana cara menentukan teknik penerjemahan mana yang dapat meningkatkan kualitas terjemahan pada frasa idiomatis?', elemen: '-', kunci: 'Menentukan teknik yang tepat dilakukan dengan menilai kesepadanan makna dan keluwesan dalam bahasa sasaran.' },
  { id: 's4_q5', pertanyaan: 'Apa perbedaan antara metode adaptasi dan terjemahan bebas?', elemen: 'M.74PEN01.008.1 E2/KUK 2.2', kunci: 'Adaptasi paling jauh dari teks sumber dan sering digunakan pada puisi/drama, sedangkan terjemahan bebas mempertahankan makna tapi menulis ulang dengan cara yang luwes.' }
];
...
export interface FormFRIA07Props {
  ...
  step4Answers?: Record<string, { answer: string; achievement: boolean | null }>;
  onAnswerChange?: (id: string, field: 'answer' | 'achievement', value: any) => void;
  onStep4Change?: (id: string, field: 'answer' | 'achievement', value: any) => void;
  umpanBalik?: string;
  ...
  onPenyusunStep4Change?: (penyusun: Array<{ nama: string; noMet: string; ttdTanggal: string }>) => void;
  ...
  onValidatorStep4Change?: (validator: Array<{ nama: string; noMet: string; ttdTanggal: string }>) => void;
  readOnly?: boolean;
  isAsesi?: boolean;
  showHeader?: boolean;
  onNext?: () => void;
...
export function FormFRIA07(props: FormFRIA07Props) {
  const questions = props.step4Questions || props.questions || DEFAULT_STEP4_QUESTIONS;

  const [localAnswers, setLocalAnswers] = useState<Record<string, { answer: string; achievement: boolean | null }>>({});
  const [localUmpanBalik, setLocalUmpanBalik] = useState('');
  const [localAsesiName, setLocalAsesiName] = useState(props.asesmenData?.nama || 'Ahmad Supriyadi');
  const [localAsesiSig, setLocalAsesiSig] = useState('');
  const [localAsesiDate, setLocalAsesiDate] = useState('');
  const [localAsesorName, setLocalAsesorName] = useState(props.asesmenData?.asesor || 'Ichsan Taufik');
  ...
  const defaultPenyusun = [
    { nama: props.asesmenData?.asesor || 'Ichsan Taufik', noMet: '', ttdTanggal: props.asesmenData?.tanggal || '11 Oktober 2024' },
  ...
  const umpanBalik = props.umpanBalikStep4 !== undefined ? props.umpanBalikStep4 : (props.umpanBalik !== undefined ? props.umpanBalik : localUmpanBalik);
  const asesiName = props.asesiNameStep4 !== undefined ? props.asesiNameStep4 : (props.asesiName !== undefined ? props.asesiName : localAsesiName);
  const asesiSignature = props.asesiSignatureStep4 !== undefined ? props.asesiSignatureStep4 : (props.asesiSignature !== undefined ? props.asesiSignature : localAsesiSig);
  const asesiDate = props.asesiDateStep4 !== undefined ? props.asesiDateStep4 : (props.asesiDate !== undefined ? props.asesiDate : localAsesiDate);
  const asesorName = props.asesorNameStep4 !== undefined ? props.asesorNameStep4 : (props.asesorName !== undefined ? props.asesorName : localAsesorName);
  const asesorReg = props.asesorRegStep4 !== undefined ? props.asesorRegStep4 : (props.asesorReg !== undefined ? props.asesorReg : localAsesorReg);
  const asesorSignature = props.asesorSignatureStep4 !== undefined ? props.asesorSignatureStep4 : (props.asesorSignature !== undefined ? props.asesorSignature : localAsesorSig);
  const asesorDate = props.asesorDateStep4 !== undefined ? props.asesorDateStep4 : (props.asesorDate !== undefined ? props.asesorDate : localAsesorDate);
  ...
  const handleAnswerChangeInternal = (id: string, field: 'answer' | 'achievement', val: any) => {
  ...
  const handleAsesiSignatureChange = (val: string) => {
    if (props.onAsesiSignatureStep4Change) props.onAsesiSignatureStep4Change(val);
  ...
  const handleAsesorSignatureChange = (val: string) => {
    if (props.onAsesorSignatureStep4Change) props.onAsesorSignatureStep4Change(val);
  ...
  const handlePenyusunChangeInternal = (idx: number, field: string, val: string) => {
  ...
  const handleValidatorChangeInternal = (idx: number, field: string, val: string) => {
  ...
  const lingkupList = [
    { title: "1. Pengumpulan Kebutuhan Teknis Pengguna yang Menggunakan Jaringan", qs: [questions[0], questions[1]].filter(Boolean) },
    { title: "2. Pengumpulan Data Peralatan Jaringan Dengan Teknologi yang Sesuai", qs: [questions[2], questions[3]].filter(Boolean) },
    { title: "3. Perancangan Topologi Jaringan", qs: [questions[4], questions[5]].filter(Boolean) },
    { title: "4. Perancangan Pengalamatan Jaringan", qs: [questions[6], questions[7]].filter(Boolean) },
    { title: "5. Penentuan Spesifikasi Perangkat Jaringan", qs: [questions[8], questions[9]].filter(Boolean) }
    ...
    <div className="animate-in fade-in duration-300">
        ...
        <div className="p-4 space-y-4">
          <div>
            ...
            <ul className="list-disc pl-5 space-y-1 text-slate-800">
              ...
              <li>Buatlah pertanyaan lisan yang dapat mencakupi penguatan informasi berdasarkan KUK, batasan variabel, pengetahuan dan keterampilan esensial, sikap dan aspek kritis.</li>
              <li>Perkiraan jawaban dapat diisikan pada baris kunci jawaban.</li>
              <li>Tanggapan/penilaian dapat diisi dengan centang pada kolom Asesi "Ya" atau "Tidak".</li>
              <li>Dibutuhkan justifikasi profesional asesor untuk memutuskan hal ini.</li>
            </ul>
          </div>
          <div className="flex flex-col sm:flex-row border border-slate-300">
            <div className="w-full sm:w-1/4 p-2 font-bold border-b sm:border-b-0 sm:border-r border-slate-300 bg-white">Instruksi:</div>
              ...
              <table className="w-full border-collapse min-w-[600px]">
                <thead>
                  <tr className="bg-white border-b border-slate-300 text-center">
                    <th className="border border-slate-300 p-2 w-12" rowSpan={2}>No.</th>
                    <th className="border border-slate-300 p-2" rowSpan={2}>Pertanyaan</th>
                    <th className="border border-slate-300 p-2 w-32" colSpan={2}>Pencapaian</th>
                  </tr>
                ...
                <tbody>
                  {lingkup.qs.map((q, qIdx) => (
                    <React.Fragment key={q.id}>
                      <tr>
                        <td className="border border-slate-300 p-2 text-center align-top" rowSpan={3}>{qIdx + 1}.</td>
                        <td className="border border-slate-300 p-3 align-top space-y-1">
                          <div className="font-bold">Pertanyaan:</div>
                          {q.elemen !== '-' && <div className="text-slate-700 whitespace-pre-wrap">{q.elemen.replace(';', '\n')}</div>}
                          <div>{q.pertanyaan}</div>
                        </td>
                        <td className="border border-slate-300 p-2 text-center align-top" rowSpan={3}>
                          <input 
                            type="radio" 
                            name={`ach4_${q.id}`} 
                            className="w-4 h-4 cursor-pointer"
                            checked={answers[q.id]?.achievement === true}
                            onChange={() => handleAnswerChangeInternal(q.id, 'achievement', true)}
                        ...
                        <td className="border border-slate-300 p-2 text-center align-top" rowSpan={3}>
                          <input 
                            ...
                            onChange={() => handleAnswerChangeInternal(q.id, 'achievement', false)}
                            disabled={props.readOnly || props.isAsesi}
                          />
                        </td>
                      </tr>
                      ...
                      <tr>
                        <td className="border border-slate-300 p-3 align-top">
                          ...
                          <textarea 
                            disabled={props.readOnly || props.isAsesi}
                            className="w-full outline-none bg-transparent min-h-[80px] resize-y"
                            placeholder="Ketik jawaban asesi..."
                            value={answers[q.id]?.answer || ''}
                            onChange={(e) => handleAnswerChangeInternal(q.id, 'answer', e.target.value)}
                          />
                        </td>
                      </tr>
                    </React.Fragment>
      ...
      <div className="mt-8 border border-slate-300 bg-white text-sm">
        <table className="w-full border-collapse">
          <tbody>
            <tr>
              <td className="border border-slate-300 p-3 font-bold w-1/3 align-top">Umpan balik untuk asesi</td>
              <td className="border border-slate-300 p-3 align-top space-y-2">
                <div>Aspek pengetahuan seluruh unit kompetensi yang diujikan (tercapai/ belum tercapai)*</div>
                <div className="flex gap-2">
                  <span>:</span>
                  <textarea 
                    disabled={props.readOnly || props.isAsesi}
                    className="w-full outline-none bg-transparent min-h-[60px]" 
        ...
        <table className="w-full border-collapse">
          <tbody>
            <tr className="bg-white">
              <td className="border border-slate-300 p-2 font-bold" colSpan={3}>ASESI :</td>
            </tr>
            <tr>
            ...
            <tr>
              <td className="border border-slate-300 p-2">Tanda tangan dan Tanggal</td>
              ...
              <td className="border border-slate-300 p-2">
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4">
                  ...
                  >
                    {asesiSignature ? (
                      <img src={asesiSignature} alt="TTD Asesi" className="h-full object-contain" />
                    ) : (
                      <span className="text-gray-400 text-sm">Klik untuk tanda tangan</span>
                    )}
                  </div>
                  <input 
                    id="date-input"
            ...
            <tr className="bg-white">
              <td className="border border-slate-300 p-2 font-bold" colSpan={3}>ASESOR :</td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td className="border border-slate-300 p-2">
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4">
                  ...
                  >
                    {asesorSignature ? (
                      <img src={asesorSignature} alt="TTD Asesor" className="h-full object-contain" />
                    ) : (
                      <span className="text-gray-400 text-sm">Klik untuk tanda tangan</span>
                    )}
                  </div>
        ...
        <div className="overflow-x-auto">
          ...
          <thead>
            <tr className="bg-white border-b border-slate-300 text-center">
              ...
              <th className="border border-slate-300 p-2 w-1/4">TANDA TANGAN DAN TANGGAL</th>
            </tr>
          </thead>
          <tbody>
            {penyusun.map((p, idx) => (
              <tr key={'p4-'+idx}>
                {idx === 0 && <td className="border border-slate-300 p-2 text-center align-top font-bold" rowSpan={penyusun.length}>PENYUSUN</td>}
                ...
                <td className="border border-slate-300 p-2">
                  <input 
                    ...
                    className="w-full outline-none bg-transparent" 
                    value={p.nama} 
                    onChange={(e) => handlePenyusunChangeInternal(idx, 'nama', e.target.value)} 
                  />
                </td>
                <td className="border border-slate-300 p-2">
                  <input 
                    ...
                    value={p.noMet} 
                    onChange={(e) => handlePenyusunChangeInternal(idx, 'noMet', e.target.value)} 
                  />
                </td>
                <td className="border border-slate-300 p-2">
                  <input 
                    ...
                    className="w-full outline-none bg-transparent text-center" 
                    value={p.ttdTanggal} 
                    onChange={(e) => handlePenyusunChangeInternal(idx, 'ttdTanggal', e.target.value)} 
                  />
                </td>
              </tr>
            ))}
            {validator.map((v, idx) => (
              <tr key={'v4-'+idx}>
                {idx === 0 && <td className="border border-slate-300 p-2 text-center align-top font-bold" rowSpan={validator.length}>VALIDATOR</td>}
                <td className="border border-slate-300 p-2 text-center">{idx + 1}</td>
                <td className="border border-slate-300 p-2">
                  <input 
                    type="text" 
                    ...
                    className="w-full outline-none bg-transparent" 
                    value={v.nama} 
                    onChange={(e) => handleValidatorChangeInternal(idx, 'nama', e.target.value)} 
                  />
                </td>
                <td className="border border-slate-300 p-2">
                  <input 
                    ...
                    className="w-full outline-none bg-transparent text-center" 
                    value={v.noMet} 
                    onChange={(e) => handleValidatorChangeInternal(idx, 'noMet', e.target.value)} 
                  />
                </td>
                <td className="border border-slate-300 p-2">
                  <input 
                    ...
                    className="w-full outline-none bg-transparent text-center" 
                    value={v.ttdTanggal} 
                    onChange={(e) => handleValidatorChangeInternal(idx, 'ttdTanggal', e.target.value)} 
                  />
                </td>
              </tr>
            ))}
      ...
      {(props.onPrev || props.onNext) && (
        <div className="mt-8 flex justify-between items-center">
          ...
          {props.onNext && (
            ...
            >
              {props.nextLabel || 'Lanjut Finalisasi'} <ChevronRight size={16} />
            </button>
          )}
        </div>
      )}
