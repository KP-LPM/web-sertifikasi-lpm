...
export function SignatureModal({ isOpen, onClose, title, onSave, initialSignature }: SignatureModalProps) {
  ...
  const resizeCanvas = () => {
    if (sigRef.current && containerRef.current) {
      ...
      sigRef.current.clear(); // necessary after resize
      if (initialSignature) {
        sigRef.current.fromDataURL(initialSignature);
      }
    }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-150">
        ...
        <div className="p-4">
          <div ref={containerRef} className="border border-gray-300 rounded-lg overflow-hidden bg-white w-full h-48 sm:h-64 relative">
            <SignatureCanvas 
              ref={sigRef}
