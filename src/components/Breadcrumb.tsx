...
export function Breadcrumb({ className = "" }: { className?: string }) {
  const { user, currentView, setCurrentView, extraCrumbs } = useAppContext();
  ...
  const getViewConfig = (): ViewConfig => {
    ...
    // Assessor Views
    if (user.role === 'asesor') {
      const candidatesCrumb: Crumb = { label: 'Daftar Asesmen', view: 'candidates' };
      const historyCrumb: Crumb = { label: 'Riwayat Asesmen', view: 'history-asesmen' };
      const configCrumb: Crumb = { label: 'Konfigurasi Pertanyaan', view: 'konfigurasi-pertanyaan' };
      ...
      switch (currentView) {
        ...
        case 'assessment-form':
          return {
            crumbs: [homeCrumb, candidatesCrumb, { label: 'Form Asesmen', view: 'assessment-form' }]
          };
        case 'jadwalkan-online':
          return {
            crumbs: [homeCrumb, candidatesCrumb, { label: 'Jadwalkan Online', view: 'jadwalkan-online' }]
          };
        case 'penilaian-online':
          return {
            crumbs: [homeCrumb, candidatesCrumb, { label: 'Penilaian Online', view: 'penilaian-online' }]
          };
        case 'finalization':
          return {
            crumbs: [homeCrumb, candidatesCrumb, { label: 'Finalisasi', view: 'finalization' }]
          };
        ...
        case 'detail-riwayat-asesmen':
          return {
            crumbs: [homeCrumb, historyCrumb, { label: 'Detail Riwayat', view: 'detail-riwayat-asesmen' }]
          };
        case 'verifikasi-apl02':
          return {
            crumbs: [homeCrumb, { label: 'Verifikasi Berkas', view: 'verifikasi-apl02' }]
          };
        case 'verifikasi-portofolio':
          return {
            crumbs: [homeCrumb, { label: 'Verifikasi Portofolio', view: 'verifikasi-portofolio' }]
          };
        case 'verifikasi-banding':
          return {
            crumbs: [homeCrumb, { label: 'Verifikasi Banding', view: 'verifikasi-banding' }]
          };
        ...
        case 'tambah-konfigurasi-pertanyaan':
          return {
            crumbs: [homeCrumb, configCrumb, { label: 'Buat Paket Soal', view: 'tambah-konfigurasi-pertanyaan' }]
          };
        case 'ubah-konfigurasi-pertanyaan':
          return {
            crumbs: [homeCrumb, configCrumb, { label: 'Ubah Paket Soal', view: 'ubah-konfigurasi-pertanyaan' }]
          };
        case 'detail-konfigurasi-pertanyaan':
          return {
            crumbs: [homeCrumb, configCrumb, { label: 'Detail Paket Soal', view: 'detail-konfigurasi-pertanyaan' }]
          };
        case 'profile':
          return {
            crumbs: [homeCrumb, { label: 'Profil Saya', view: 'profile' }]
    ...
    if (user.role === 'asesi') {
      switch (currentView) {
        case 'apply':
          return {
            crumbs: [homeCrumb, { label: 'Pengajuan Skema', view: 'apply', onClick: () => { setCurrentView('apply'); window.dispatchEvent(new CustomEvent('reset-eform')); } }]
          };
        case 'history':
          return {
            crumbs: [homeCrumb, { label: 'Riwayat & Sertifikat', view: 'history' }]
          };
        case 'ujian':
          return {
    ...
    if (user.role === 'direktur' || user.role === 'manajer') {
      switch (currentView) {
        case 'pleno':
          return {
            crumbs: [homeCrumb, { label: 'Sidang Pleno', view: 'pleno' }]
          };
        case 'profile':
          return {
            crumbs: [homeCrumb, { label: 'Profil Saya', view: 'profile' }]
            ...
            {isLast ? (
              ...
              <button
                ...
                onClick={() => {
                  ...
                  else if (crumb.view) setCurrentView(crumb.view);
                }}
