...
export default function AsesiOverviewPage() {
  ...
  const filteredAssessments = REGISTERED_ASSESSMENTS.filter((item) => {
    ...
    const matchesDate = !dateFilter || item.tanggalAsesmen === dateFilter.split('-').reverse().join('/');
    
    return matchesSearch && matchesStatus && matchesDate;
  });

  const totalPages = Math.ceil((filteredAssessments?.length || 0) / itemsPerPage);
  const currentRecords = filteredAssessments.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  ...
  const selesaiCount = REGISTERED_ASSESSMENTS.filter(a => a.statusAsesmen === 'Selesai').length;
  ...
  return (
    <div className="space-y-6 pb-24 text-sm text-gray-700">
      ...
      <div className="space-y-2">
        <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">Overview</h3>
      ...
      <section className="bg-white rounded-lg shadow-xs border border-gray-100 overflow-hidden">
              ...
              {filteredAssessments.length > 0 ? (
                currentRecords.map((item, idx) => (
                  <tr key={item.id} className="group/row hover:bg-[#F9FAFC] transition-colors">
                    ...
                    <td className="px-6 py-4 text-xs text-gray-500 font-medium" title={item.alamat}>
                      {item.alamat}
                    </td>
                    ...
                    <td className="px-6 py-4 text-xs md:text-sm">
                      {item.linkVirtualMeeting && item.linkVirtualMeeting !== '-' ? (
                    ...
                    <td className="px-6 py-4 text-xs md:text-sm">{getRekomendasiBadge(item.rekomendasi)}</td>
                    <td className="px-6 py-4 text-xs md:text-sm">{getStatusBadge(item.statusAsesmen)}</td>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        <button 
                          ...
                          className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all shadow-2xs border ${
                            (item.tuk === 'Mandiri' && item.statusAsesmen === 'Terjadwal')
                ...
                <tr>
                  <td colSpan={10} className="px-6 py-12 text-center text-xs md:text-sm text-gray-400 font-medium">
                    Tidak ada kegiatan asesmen yang cocok dengan pencarian atau filter Anda.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {totalPages >= 1 && (
          <div className="p-4 px-6 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-gray-500 font-medium bg-gray-50/50 rounded-b-xl">
            <span>
              Menampilkan <span className="font-semibold text-slate-700">{(currentPage - 1) * itemsPerPage + 1}</span> hingga <span className="font-semibold text-slate-700">{Math.min(currentPage * itemsPerPage, filteredAssessments.length)}</span> dari <span className="font-semibold text-slate-700">{filteredAssessments.length}</span> entri
            </span>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
            ...
            <div className="p-5 space-y-4 font-medium text-sm">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <span className="col-span-2 text-slate-900">{selectedAssessment.alamat}</span>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <span className="text-slate-500 font-semibold">Tanggal Asesmen</span>
                <span className="col-span-2 text-slate-900">{selectedAssessment.tanggalAsesmen}</span>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <span className="text-slate-500 font-semibold">Asesor</span>
                <span className="col-span-2 text-slate-900">{selectedAssessment.asesor}</span>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <span className="text-slate-500 font-semibold">Virtual Meeting</span>
                <span className="col-span-2 text-slate-900">
                  {selectedAssessment.linkVirtualMeeting && selectedAssessment.linkVirtualMeeting !== '-' ? (
                    <a href={selectedAssessment.linkVirtualMeeting} target="_blank" rel="noreferrer" className="text-[#008BE3] hover:underline">
              ...
              <div className="grid grid-cols-3 gap-2 items-center">
                ...
                <span className="col-span-2">{selectedAssessment.rekomendasi !== '-' ? getRekomendasiBadge(selectedAssessment.rekomendasi) : '-'}</span>
              </div>
            </div>
          </div>
        </div>
