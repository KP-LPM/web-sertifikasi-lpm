...
import { Search, Plus, Edit, Trash2, CheckCircle, Clock, Eye, Users, ArrowLeft } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import SignatureCanvas from 'react-signature-canvas';
import { useRef, useEffect } from 'react';
import { EFormApl01 } from '../asesi/EFormApl01';
import { EFormApl02 } from '../asesi/EFormApl02';
...
export function UsersManagement() {
  ...
  const [users, setUsers] = useState<UserItem[]>([
    { id: '1', initial: 'AH', name: 'Ahmad Hidayat', email: 'ahmad.h@student.uin.ac.id', role: 'Asesi', status: 'Menunggu Verifikasi' },
    { id: '2', initial: 'SR', name: 'Dr. Siti Rohmah', email: 'siti.r@lecturer.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-1', initial: 'IT', name: 'Ichsan Taufik', email: 'ichsan.taufik@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-2', initial: 'AK', name: 'Aceng Abdul Kodir', email: 'aceng.kodir@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-3', initial: 'SF', name: 'Susanti Ainul Fitri', email: 'susanti.fitri@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-4', initial: 'SM', name: 'M Sandi Marta', email: 'sandi.marta@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-5', initial: 'GS', name: 'Gina Sakinah', email: 'gina.sakinah@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-6', initial: 'EW', name: 'Elis Ratna Wulan', email: 'elis.wulan@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-7', initial: 'AS', name: 'Asep Abdul Sahid', email: 'asep.sahid@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-8', initial: 'SA', name: 'Siti Alia', email: 'siti.alia@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-9', initial: 'AF', name: 'Azmi Fasa', email: 'azmi.fasa@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-10', initial: 'CS', name: 'Cucu Susilawati', email: 'cucu.susilawati@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-11', initial: 'FW', name: 'Fitri Pebriani Wahyu', email: 'fitri.wahyu@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-12', initial: 'TR', name: 'Tina Dewi Rosahdi', email: 'tina.rosahdi@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-13', initial: 'UJ', name: 'Ucu Julita', email: 'ucu.julita@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-14', initial: 'AM', name: 'Acep Muslim', email: 'acep.muslim@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-15', initial: 'IK', name: 'Izzah Faizah Siti Rusydati Khaerani', email: 'izzah.faizah@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-16', initial: 'MA', name: 'Muhammad Alfan', email: 'muhammad.alfan@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-17', initial: 'EA', name: 'Erlan Aditya Ardiansyah', email: 'erlan.ardiansyah@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-18', initial: 'DG', name: 'Dian Rachmat Gumelar', email: 'dian.gumelar@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-19', initial: 'RN', name: 'Reza Fauzi Nazar', email: 'reza.nazar@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-20', initial: 'RS', name: 'Rini Sulastri', email: 'rini.sulastri@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-21', initial: 'YM', name: 'Yadi Mardiansyah', email: 'yadi.mardiansyah@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-22', initial: 'DY', name: 'Dayudin', email: 'dayudin@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-23', initial: 'WU', name: 'Wisnu Uriawan', email: 'wisnu.uriawan@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' },
    { id: 'asr-24', initial: 'MR', name: 'M. Ridha Taufiq Rahman', email: 'ridha.rahman@lsp.uin.ac.id', role: 'Asesor', status: 'Terverifikasi' }
  ]);
  ...
  const lspSignatureRef = useRef<any>(null);
  const [verificationForm, setVerificationForm] = useState({ rekomendasi: 'Diterima', catatan: '' });
  const [apl01FormData, setApl01FormData] = useState<any>({ isAdmin: true });
  const [apl02FormData, setApl02FormData] = useState<any>({ isAdmin: true });
  const [selectedAsesorId, setSelectedAsesorId] = useState<string>('');

  // Form State
  const [formData, setFormData] = useState({ name: '', email: '', role: 'Asesor' });
  ...
  const confirmVerify = () => {
    if (userToVerify) {
      ...
      let currentLspUrl = userToVerify.verificationData?.lspSignatureUrl || null;
      ...
      const newVerificationData = {
        rekomendasi: activeVerifyTab === 'apl01' ? apl01FormData.rekomendasi || 'Diterima' : verificationForm.rekomendasi,
        catatan: activeVerifyTab === 'apl01' ? apl01FormData.catatan || '' : verificationForm.catatan,
      ...
      setUsers(users.map(u => u.id === userToVerify.id ? { 
        ...u, 
    ...
    if (user.verificationData) {
      ...
      setTimeout(() => {
        if (user.verificationData?.adminSignatureUrl && signatureRef.current) {
           signatureRef.current.fromDataURL(user.verificationData.adminSignatureUrl);
        }
        if (user.verificationData?.lspSignatureUrl && lspSignatureRef.current) {
           lspSignatureRef.current.fromDataURL(user.verificationData.lspSignatureUrl);
        }
      }, 50);
    } else {
  ...
  const handleVerify = (id: string) => {
    setUsers(users.map(u => u.id === id ? { ...u, status: 'Terverifikasi' } : u));
  };
  ...
  if (isVerifyModalOpen && userToVerify) {
    ...
    const isApl01Valid = !!apl01FormData.ttdAdmin && (totalReqs === 0 || checkedReqs === totalReqs);
      ...
      <div className="min-h-screen bg-[#F8F9FC] p-4 md:p-8 pb-24 text-sm text-gray-700">
        <div className="flex flex-col gap-6">
          <div className="flex items-center gap-4">
            <button 
              onClick={() => { setIsVerifyModalOpen(false); setUserToVerify(null); }}
            ...
            <div className="min-w-0">
              <h2 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight">Tinjauan Verifikasi Berkas</h2>
              <p className="text-xs text-gray-500 font-medium mt-1">Periksa detail dokumen {userToVerify.name} sebelum menyetujui.</p>
            </div>
          </div>
              ...
              <div className="border-b border-gray-100 bg-slate-50 p-4">
                <div className="flex items-center justify-between">
                   <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-slate-900">{userToVerify.name}</p>
                      <p className="text-xs text-gray-500 font-medium">{userToVerify.email}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold ${activeVerifyTab === 'apl01' ? 'bg-[#008BE3] text-white shadow-sm' : 'bg-slate-200 text-slate-500'}`}>
                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] ${activeVerifyTab === 'apl01' ? 'bg-white text-[#008BE3]' : 'bg-slate-300 text-slate-500'}`}>1</span>
                      FR.APL.01
                    </div>
                    <div className="w-8 h-px bg-slate-300"></div>
                    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold ${activeVerifyTab === 'apl02' ? 'bg-[#008BE3] text-white shadow-sm' : 'bg-slate-200 text-slate-500'}`}>
                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] ${activeVerifyTab === 'apl02' ? 'bg-white text-[#008BE3]' : 'bg-slate-300 text-slate-500'}`}>2</span>
                      FR.APL.02
                    </div>
                  </div>
                </div>
            ...
            <div className="p-6 space-y-4">
              {userToVerify.role === 'Asesi' ? (
                <>
                  {activeVerifyTab === 'apl01' ? (
                    <div className="space-y-6">
                      <EFormApl01 formData={apl01FormData} onChange={setApl01FormData} />
                    </div>
                  ) : (
                    <div className="space-y-4">
                      <EFormApl02 
                        ...
                        allData={{
                          "UK.01 - Menyiapkan pemeriksaan dokumen": { name: "Ijazah.pdf" },
                          "UK.01 - Melakukan pemeriksaan kesesuaian bukti": { name: "Portofolio.pdf" },
                          "UK.02 - Menyusun ringkasan bukti": { name: "Transkrip.pdf" }
                        }} 
                      />
                    </div>
                  )}
                ...
                <div className="grid gap-3">
                  <div className="min-w-0">
                    <p className="text-[10px] uppercase font-bold text-slate-500 mb-0.5">Bidang Keahlian / Skema</p>
                    <p className="text-sm font-bold text-slate-800">Rekayasa Perangkat Lunak</p>
                  </div>
                  <div className="min-w-0">
                    <p className="text-[10px] uppercase font-bold text-slate-500 mb-0.5">No. Registrasi / NIP</p>
                    <p className="text-sm font-bold text-slate-800">MET.000.12345.2023</p>
                  </div>
                  <div className="min-w-0">
                    <p className="text-[10px] uppercase font-bold text-slate-500 mb-0.5">Sertifikat Asesor</p>
                    <p className="text-xs font-bold text-green-700 flex items-center gap-1.5"><CheckCircle size={14} /> Terlampir</p>
                  </div>
                </div>
              )}
            </div>
                ...
                <>
                  {!apl02FormData.ttdAsesor && userToVerify.role === 'Asesi' ? (
                    ...
                    <button 
                      ...
                      disabled={userToVerify?.role === 'Asesi' && activeVerifyTab === 'apl02' && (!apl01FormData.ttdAdmin || !apl02FormData.ttdAsesor)}
            ...
            >
              ...
              <div className="flex-1 p-6 bg-slate-100 flex items-center justify-center">
                <div className="w-full h-full bg-white border border-slate-200 rounded shadow-sm flex flex-col items-center justify-center text-slate-400">
                  ...
                  <p className="font-medium text-slate-500">Pratinjau Dokumen PDF / Gambar</p>
                  <p className="text-xs mt-2">Ini adalah simulasi tampilan dokumen.</p>
                </div>
              </div>
            </motion.div>
          </div>
         ...
         <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray-500 font-medium tracking-wider uppercase leading-[16px]">
              Verifikasi dokumen pendaftaran APL.01 &amp; APL.02 asesi dan penugasan asesor.
            </p>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#0F172A] border-b border-[#0F172A]">
                <th className="px-6 py-4 text-xs font-bold text-white/90 uppercase tracking-wider">Profil Pengguna</th>
                <th className="px-6 py-4 text-xs font-bold text-white/90 uppercase tracking-wider">Peran</th>
                <th className="px-6 py-4 text-xs font-bold text-white/90 uppercase tracking-wider">Status Verifikasi</th>
                {!readOnly && (
                  <th className="px-6 py-4 text-xs font-bold text-white/90 uppercase tracking-wider text-left sticky right-0 bg-[#0F172A] z-10 border-l border-white/10 shadow-[-6px_0_15px_-4px_rgba(0,0,0,0.06)]">Aksi</th>
                )}
              </tr>
            </thead>
              ...
              {filteredUsers.map((user) => (
                <tr key={user.id} className="group/row hover:bg-[#F9FAFC] transition-colors">
                  <td className="px-6 py-4">
                     <div className="flex items-center gap-3 min-w-0">
                      <div className="w-9 h-9 rounded-lg bg-sky-50 text-[#008BE3] flex items-center justify-center font-bold text-xs border border-sky-100">
                        {user.initial}
                      </div>
                      <div className="min-w-0">
                  ...
                  <td className="px-6 py-4">
                    <span className={`border px-2.5 py-1 rounded text-xs font-bold inline-block ${
                      user.role === 'Asesor' 
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 rounded text-[10px] font-bold uppercase tracking-wider inline-flex items-center gap-1.5 border ${
                      user.status === 'Terverifikasi' 
                        ? 'bg-green-50 text-green-700 border-green-200' 
                        : 'bg-amber-50 text-amber-700 border-amber-200'
                    }`}>
                      {user.status === 'Terverifikasi' ? <CheckCircle size={12} className="stroke-[2.5]" /> : <Clock size={12} className="stroke-[2.5]" />} 
                      {user.status}
                    </span>
                  </td>
                  {!readOnly && (
                  <td className="px-6 py-4 text-left space-x-2 sticky right-0 bg-white group-hover/row:bg-[#F9FAFC] z-10 border-l border-gray-100 shadow-[-6px_0_15px_-4px_rgba(0,0,0,0.02)]">
                    <button onClick={() => openEditModal(user)} className="p-1.5 text-gray-400 hover:text-[#008BE3] hover:bg-sky-50 rounded transition-colors"><Edit size={16} /></button>
                    <button onClick={() => { setSelectedUser(user); setIsDeleteModalOpen(true); }} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors"><Trash2 size={16} /></button>
                    {user.status !== 'Terverifikasi' && (
                      <button onClick={() => openVerifyModal(user)} className="bg-sky-50 text-[#008BE3] border border-sky-200 px-2.5 py-1 rounded text-xs font-bold hover:bg-[#008BE3] hover:text-white hover:border-[#008BE3] transition-colors ml-2">Verifikasi</button>
                    )}
                  </td>
                  )}
                </tr>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
            ...
            >
              <div className="p-5 border-b border-gray-100">
                <h3 className="text-lg font-black text-slate-900">{isEditModalOpen ? 'Edit Pengguna' : 'Tambah Pengguna Baru'}</h3>
                <p className="text-xs text-gray-500 mt-1">{isEditModalOpen ? 'Ubah data pengguna ini.' : 'Tambahkan akun pengguna untuk Admin atau Asesor.'}</p>
              </div>
              <div className="p-5 space-y-4">
                ...
                <div className="min-w-0">
                  <label className="block text-xs font-bold text-slate-700 mb-1.5">Peran</label>
                  <select 
                    value={formData.role}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="p-6 text-center">
                ...
                <p className="text-sm text-gray-500">Apakah Anda yakin ingin menghapus {selectedUser?.name}? Tindakan ini tidak dapat dibatalkan.</p>
              </div>
