...
import { 
  ...
  BadgeCheck, X, Trash2, Upload, Save, Eye, EyeOff
} from 'lucide-react';
...
export default function Login() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showNotification('Terjadi kesalahan pada server saat mendaftar.', 'error');
    }
    setIsLoading(false);
  };
  ...
  if (isForgotPasswordView) {
    return (
      <div 
        ...
        style={{ backgroundImage: "linear-gradient(rgba(255, 255, 255, 0.85), rgba(255, 255, 255, 0.8)), url('/bg-lpm.jpeg')" }}
      >
        {/* POP-UP NOTIFIKASI */}
        {notification.show && (
          <motion.div
            ...
            className={`fixed top-6 left-1/2 -translate-x-1/2 z-[100] px-5 py-3 rounded-xl shadow-lg flex items-center gap-3 border backdrop-blur-md ${
              notification.type === 'success' ? 'bg-emerald-50/90 border-emerald-200 text-emerald-800' : 'bg-rose-50/90 border-rose-200 text-rose-800'
            }`}
          >
            {notification.type === 'success' ? <BadgeCheck size={20} className="text-emerald-500" /> : <X size={20} className="text-rose-500 bg-rose-100 rounded-full p-0.5" />}
        ...
        <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-lg bg-white rounded-xl border border-slate-200/90 shadow-sm p-6 md:p-10">
          ...
          <form className="space-y-5" onSubmit={handleForgotPassword}>
            <div>
              ...
              <input type="email" className="text-black w-full px-4 py-2 text-xs border rounded-lg outline-none focus:border-[#008BE3]" placeholder="contoh@domain.com" required />
            </div>
            <div className="flex gap-3">
              <button type="button" onClick={() => setIsForgotPasswordView(false)} className="text-black w-full px-5 py-2 border rounded-lg text-xs font-bold hover:bg-slate-50">Kembali</button>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-xl border border-slate-200 shadow-sm w-full max-w-5xl my-auto">
          <div className="p-6 md:p-8 space-y-6">
            <div className="flex items-center gap-3 border-b border-slate-100 pb-5">
              <button type="button" onClick={() => setIsLoginView(true)} className="text-black p-1.5 hover:bg-slate-50 rounded-lg border border-slate-200">
                <ArrowLeft size={16} />
              </button>
              <div>
            ...
            <form className="space-y-6" onSubmit={handleRegister}>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div>
                    ...
                    <input type="text" name="username" className="text-black w-full px-3 py-2 text-xs border rounded-lg outline-none focus:border-[#008BE3]" placeholder="Masukkan nama pengguna" required />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">Email Aktif</label>
                    <input type="email" name="email" className="text-black w-full px-3 py-2 text-xs border rounded-lg outline-none focus:border-[#008BE3]" placeholder="Masukkan email aktif" required />
                  </div>
                  <div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input type="text" name="nik" className="text-black w-full px-3 py-2 text-xs border rounded-lg outline-none focus:border-[#008BE3]" placeholder="Masukkan NIK" required />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">Nama Lengkap</label>
                    <input type="text" name="nama_lengkap" className="text-black w-full px-3 py-2 text-xs border rounded-lg outline-none focus:border-[#008BE3]" placeholder="Masukkan nama lengkap" required />
                  </div>

                  {mode === 'asesor' && (
                    <div>
                      <label className="block text-xs font-bold text-slate-700 mb-1.5">Nomor Registrasi/MET</label>
                      <input type="text" name="nomor_registrasi_met" className="text-black w-full px-3 py-2 text-xs border rounded-lg outline-none focus:border-[#008BE3]" placeholder="Masukkan nomor registrasi/MET" required />
                    </div>
                  )}

                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">Tempat Lahir</label>
                    <input type="text" name="tempat_lahir" className="text-black w-full px-3 py-2 text-xs border rounded-lg outline-none focus:border-[#008BE3]" placeholder="Masukkan tempat lahir" required />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">Tanggal Lahir</label>
                    <input type="date" name="tanggal_lahir" className="text-black w-full px-3 py-2 text-xs border rounded-lg outline-none focus:border-[#008BE3]" placeholder="Masukkan tanggal lahir" required />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-2">Jenis Kelamin</label>
                    <div className="flex gap-6 py-1">
                      <label className="text-black flex items-center text-xs font-bold"><input type="radio" name="jenis_kelamin" value="Laki-laki" className="w-4 h-4" required /><span className="ml-2">Laki-laki</span></label>
                      <label className="text-black flex items-center text-xs font-bold"><input type="radio" name="jenis_kelamin" value="Perempuan" className="w-4 h-4" required /><span className="ml-2">Perempuan</span></label>
                    </div>
                  </div>
                  ...
                  {mode === 'asesi' && (
                    <div>
                      ...
                      <select name="kewarganegaraan" className="text-black w-full px-3 py-2 text-xs border rounded-lg outline-none focus:border-[#008BE3]" required>
                  ...
                  <div>
                    ...
                    <input type="tel" name="no_hp" className="text-black w-full px-3 py-2 text-xs border rounded-lg outline-none focus:border-[#008BE3]" placeholder="Masukkan nomor HP" required />
                  </div>

                  {mode === 'asesor' && (
                    <div>
                      <label className="block text-xs font-bold text-slate-700 mb-1.5">Pendidikan Terakhir</label>
                      <select name="pendidikan_terakhir" className="text-black w-full px-3 py-2 text-xs border rounded-lg outline-none focus:border-[#008BE3]" required>
                  ...
                  <div>
                    ...
                    <select name="pekerjaan" className="text-black w-full px-3 py-2 text-xs border rounded-lg outline-none focus:border-[#008BE3]" required>
                  ...
                  {mode === 'asesor' && (
                    <div>
                      ...
                      <input type="text" name="alamat_wilayah" className="text-black w-full px-3 py-2 text-xs border rounded-lg outline-none focus:border-[#008BE3]" placeholder="Masukkan alamat wilayah/kelurahan" required />
                    </div>
                  )}
              ...
              <div className="pt-5 flex flex-col sm:flex-row gap-3 sm:justify-between items-center bg-slate-50/50 p-4 rounded-lg">
                <button type="button" onClick={() => setIsLoginView(true)} className="text-black w-full sm:w-auto px-5 py-2 border bg-white rounded-lg text-xs font-bold shadow-2xs">Kembali ke Login</button>
        ...
        {isSignatureModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden">
              <div className="text-black p-4 border-b flex justify-between"><h3 className="font-bold text-sm">Buat Tanda Tangan</h3><button onClick={() => setIsSignatureModalOpen(false)}><X size={18} /></button></div>
              <div className="p-4">
              ...
              <div className="p-4 border-t flex flex-wrap gap-2 justify-between bg-slate-50">
                <div className="flex gap-2">
                  <button type="button" onClick={() => setIsSignatureModalOpen(false)} className="text-black px-4 py-2 border bg-white rounded-lg text-xs font-bold">Batal</button>
        ...
        <div className="hidden md:flex md:w-6/12 relative bg-[#0F172A]">
          ...
          <div className="relative z-20 flex flex-col justify-between p-10 h-full text-white">
            <div className="flex items-center">
              <img src="/logo-lsp.png" alt="Logo LSP UIN SGD" className="h-12 w-auto object-contain" />
            </div>
            ...
            <div className="text-[10px] text-slate-400 font-semibold">© 2026 Lembaga Sertifikasi Profesi UIN Sunan Gunung Djati.</div>
          </div>
        </div>
            ...
            <form className="space-y-4" onSubmit={handleLogin}>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="text-black w-full pl-9 pr-4 py-2 text-xs border rounded-lg outline-none focus:border-[#008BE3]" placeholder="Masukkan nama pengguna" required />
                </div>
              </div>
